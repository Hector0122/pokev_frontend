/**
 * Tokens de PokeV — copiado y adaptado de arcd_kit (../../../arcd_kit/tokens.ts),
 * el starter compartido entre todas las apps de producto (Varo, VaultGaming,
 * Vaulta, Vellum, Veya, Velody). Ver arcd_kit/README.md para el razonamiento
 * completo de qué se comparte (neutros, spacing, radios, motion, tipografía)
 * y qué es propio de cada app (primary/accent, definidos en `brands.pokev`
 * al final de este archivo).
 *
 * Uso en la app:
 *   import { createAppTheme, brands, motion, spacing, radius, type } from '../theme/tokens';
 *   const { light } = createAppTheme(brands.pokev);
 */
...
export const hues = {
  ...
  orange: '#F97316', // PokeV (acento) — Velody lo usaba como acento secundario, no de marca; aquí sí es color de marca
  pokeRed: '#EE1C25', // PokeV — rojo saturado de Pokéball, distinto a propósito del `red` (=danger, más apagado/rosado)
} as const;
...
export const type = {
  ...
  // Sin `fontWeight` a propósito: en Android, combinar `fontFamily` con un
  // Pokémon custom + `fontWeight` hace que RN busque una variante de peso
  // que no existe como archivo (solo tenemos el weight "Medium" del .ttf) y
  // cae en silencio a la fuente del sistema — verificado en dispositivo real
  // (Pixel/Nothing, RN 0.86). El .ttf de IBM Plex Mono ya es "Medium".
  label: { fontSize: 12, lineHeight: 16, fontFamily: fontFamily.mono },
  caption: { fontSize: 11, lineHeight: 14, fontFamily: fontFamily.mono },
} as const;
...
export const brands = {
  ...
  pokev: { primary: hues.pokeRed, accent: hues.orange }, // colección de cartas Pokémon — rojo de Pokéball + naranja de llama (Charizard)
